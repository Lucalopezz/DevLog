    ...
    <main className="mx-auto w-full max-w-5xl space-y-8">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Browse the issues and learnings recorded in your account.
          </p>
        </div>
      ...
      {!isPending && !isError && data?.data.length === 0 ? (
        <section className="space-y-2 rounded-xl border border-dashed p-10 text-center">
          <h2 className="font-semibold">No technical entries found</h2>
          <p className="text-sm text-muted-foreground">
            Try changing the filters or record your first technical entry.
          </p>
        </section>
      ) : null}
      ...
      {!isPending && !isError && data && data.data.length > 0 ? (
        <>
          <TechnicalEntryList entries={data.data} />
